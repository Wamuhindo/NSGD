from utils import FunctionState
import numpy as np
class FunctionInstance:
  id_counter = 0
  def __init__(self,t,cold_service_process, warm_service_process, expiration_process, cold_start_process, execution_time=0, mem=0,cpu=0,gpu=0,tpu=0,priority=1,deadline=None):
  ...
  def __str__(self):
    return f'Function(id={self.id},RAM={self.mem},CPU={self.cpu},GPU={self.gpu},TPU={self.tpu},Entered at {self.creation_time},Finished at {self.finished_at},Execution time={self.execution_time},Deadline={self.deadline},Departure={self.next_departure})'
  ...
  def init_resource_usages(self,cpu_warm=(100,100,),cpu_cold=(150,500,),cpu_idle=(5,30,),mem_warm=(1,1,),mem_cold=(1.2,2,),mem_idle=(0.1,0.5,),gpu_warm=(0,0,),gpu_cold=(0,0,),gpu_idle=(0,0,), gpu_warm_mem=(0,0,),gpu_cold_mem=(0,0,),gpu_idle_mem=(0,0,)):
    self.cpu_warm = 1       #cpu_warm
    self.cpu_cold = 3.25    #cpu_cold    
    self.cpu_idle = 0.175   #cpu_idle
    self.mem_warm = 1       #mem_warm
    self.mem_cold = 1.6     #mem_cold    
    self.mem_idle = 0.3     #mem_idle
  ...
  def get_resource_usages(self):
    if self.is_busy():
      return self.cpu_warm, self.mem_warm, np.random.uniform(*self.gpu_warm),np.random.uniform(*self.gpu_warm_mem)
    elif self.is_init_free() or self.is_init_reserved():
      return self.cpu_cold,self.mem_cold,np.random.uniform(*self.gpu_cold),np.random.uniform(*self.gpu_cold_mem)
    elif self.is_idle_on():
      return self.cpu_idle,self.mem_idle,np.random.uniform(*self.gpu_idle),np.random.uniform(*self.gpu_idle_mem)
    else:
      return 0,0,0,0
  ...
  def generate_init_departure(self, t):
    ...
    warm_service_time = self.warm_service_process.generate_trace()
    self.next_departure = self.next_init_departure if not self.is_reserved else self.next_init_departure + warm_service_time  #t + self.cold_service_process.generate_trace() # could also do this way(check with the others):self.next_init_departure + self.warm_service_process.generate_trace() 
    self.execution_time = (self.next_init_departure - t) + warm_service_time
    self.init_time = (self.next_init_departure - t)
  ...
  def update_next_termination(self):
    ...
    """
    self.next_termination = self.next_departure + np.inf #self.expiration_process.generate_trace()
  ...
  def arrival_transition(self, t):
    ...
    if self.is_cold() or self.is_busy():
        ...
        warm_service_time  = self.warm_service_process.generate_trace()
        self.next_departure = t + warm_service_time
        self.execution_time = warm_service_time
        self.update_next_termination()
    elif self.is_init_free() and not self.is_reserved():
        self.init_time = self.next_departure - t
        self.reserve()
         
      
  def update_next_transition(self, t):
    warm_service_time = self.warm_service_process.generate_trace()
    self.next_departure = self.next_init_departure + warm_service_time
    self.execution_time = warm_service_time + self.init_time
    self.update_next_termination()     
