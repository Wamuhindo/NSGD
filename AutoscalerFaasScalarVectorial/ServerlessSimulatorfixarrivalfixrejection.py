        ...
        for _ in range(exp_per_run):
            for seed in seeds:
                ...
                for node_idx, node_name in enumerate(tp_sort):
                    ...
                    print("Running node:", node_name, "with index:", node_idx)
                    if node_idx == 0:
                        computed_arr = float(config['nodes'][tp_sort[0]]['arrival_rate'])
                    else:
                        computed_arr = 0.0
                        for j in range(node_idx):
                            parent = tp_sort[j]
                            if matrix[j, node_idx] > 0 and parent in graph_results:
                                p_rej = graph_results[parent]['prob_reject']
                                parent_arr = graph_results[parent]['computed_arrival_rate']
                                computed_arr += parent_arr * (1 - p_rej) * matrix[j, node_idx]
                                
                            print("computed_arr", computed_arr, "actual ", arrival_rates[node_idx])
