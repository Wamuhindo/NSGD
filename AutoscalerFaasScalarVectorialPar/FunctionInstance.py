from AutoscalerFaasScalarVectorialPar.utils import FunctionState
import numpy as np
class FunctionInstance:
  ...
  def make_Init_Reserved(self):
    assert self.state == FunctionState.COLD, "Function must be in COLD state to be init_reserved."
    self.state = FunctionState.INIT_RESERVED
  def make_Init_Free(self):
    assert self.state == FunctionState.COLD, "Function must be in COLD state to be init_free."
  ...
  def generate_init_departure(self, t):
    ...
    self.next_departure = self.next_init_departure if not self.is_reserved else self.next_init_departure + self.warm_service_process.generate_trace()
  ...
  def arrival_transition(self, t):
    """Make an arrival transition, which causes the instance to go from IDLE to BUSY."""
  ...
  def make_transition(self):
    """Make the next internal transition."""
    if self.is_busy():
