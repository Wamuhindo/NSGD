"""
Parallel vectorial NSGD serverless simulator (Autoscaling.pdf).

Runs 2*K simulator processes in parallel:
  - K "plus" processes: simulate with theta + perturbations
  - K "minus" processes: simulate with theta - perturbations
Synchronize at tau_n boundaries using a Barrier.
The "plus_0" process computes the gradient update via Algorithm.
...
from AutoscalerFaasScalarVectorialPar.SimProcess import ExpSimProcess, ConstSimProcess, ParetoSimProcess
from AutoscalerFaasScalarVectorialPar.FunctionInstance import FunctionInstance
from AutoscalerFaasScalarVectorialPar.utils import FunctionState, SystemState
from AutoscalerFaasScalarVectorialPar.Algorithm import VectorialAutoScalingAlgorithm
...
class ServerlessSimulator:
    ...
    def __init__(self, id, arrival_process=None, warm_service_process=None,
        ...
        if expiration_process_type == "Deterministic":
            self.expiration_process = ConstSimProcess(self.opt_init[2] / K_exp, gen=exp_rng)
        else:
            self.expiration_process = ExpSimProcess(self.opt_init[2] / K_exp, gen=exp_rng)
    ...
    def set_algo_params(self, k_delta, k_gamma, opt_init, tau, K):
        self.autoscaler = VectorialAutoScalingAlgorithm(
            ...
            theta_init=opt_init, tau=tau, K=K, T=self.max_time, log_dir=self.log_dir
        )
    ...
    def warm_start_arrival(self, t, theta):
        """Handle warm start arrival. Policy 1: if #idle < theta_idle, scale up."""
        self.total_req_count += 1
        ...
        if was_idle:
            ...
            # Policy 1: if #idle-on < theta_idle, trigger scale-up
...
# ======================================================================
# Parallel SPSA worker
# ======================================================================

def run_simulator_process(sim_id, shared_params, sim_type, shared_data, exp, theta_lock, barrier):
    """Run a single simulator process with vectorial SPSA perturbation.

    Uses VectorialAutoScalingAlgorithm methods for:
      - stochastic rounding (algo.stochastic_round, algo.get_perturbed_theta_step)
      - SPSA sequences (algo.get_delta_n, algo.get_tau_n)
      - perturbation generation (algo.generate_perturbation)
    ...
    learn_mask = shared_params.get('learn_mask', [True, True, True])
    accumulate_cost = shared_params.get('accumulate_cost', True)
    ...
    sim.set_algo_params(
        ...
        opt_init=opt_init, tau=tau, K=K
    )

    algo = sim.autoscaler
    algo.set_params(K_exp=K_exp, gamma_min=gamma_min, prtb=prtb,
                    learn_mask=learn_mask, accumulate_cost=accumulate_cost)

    sim.set_progress(False)
    algo.theta_step = np.array(shared_data["opt"])

    # Stochastic rounding RNGs
    rang_stock_plus = np.random.default_rng(seed + sim_id * 10)
    rang_idle_plus = np.random.default_rng(seed + sim_id * 10)
    rang_stock_minus = np.random.default_rng(seed + sim_id * 10)
    rang_idle_minus = np.random.default_rng(seed + sim_id * 10)

    # Perturbation RNG (same seed for matching plus/minus pairs)
    rang_perturb = np.random.default_rng(seed)

    # Leader process creates its own algo for gradient/optimizer state
    leader_algo = None
    if sim_type == "plus_0":
        leader_algo = VectorialAutoScalingAlgorithm(
            N=maximum_concurrency, k_delta=k_delta, k_gamma=k_gamma,
            theta_init=opt_init, tau=tau, K=K, T=max_time, log_dir=log_dir
        )
        leader_algo.set_params(K_exp=K_exp, gamma_min=gamma_min, prtb=prtb,
                               learn_mask=learn_mask, accumulate_cost=accumulate_cost)
    ...
    while t < max_time:
        # Reset stochastic rounding RNGs each iteration
        rang_stock = np.random.default_rng(seed + sim_id * 10)
        rang_idle = np.random.default_rng(seed + sim_id * 10)

        delta_n = algo.get_delta_n(n)

        with theta_lock:
            opt = np.array(shared_data["opt"])

        # Generate perturbation via Algorithm
        perturbations = algo.generate_perturbation(rang_perturb, delta_n)
        with theta_lock:
            shared_data[f"perturbations_{sim_type}"] = perturbations.tolist()

        # Apply perturbation: plus adds, minus subtracts
        if sim_type == f"plus_{sim_id}":
            opt_delta = opt + perturbations
        else:
            opt_delta = opt - perturbations
        ...
        while steps < tau_n:
            ...
            # Compute theta_step via Algorithm's stochastic rounding
            if sim_type == f"plus_{sim_id}":
                opt_step = algo.get_perturbed_theta_step(opt_delta, rang_stock, rang_idle)
            else:
                opt_step = algo.get_perturbed_theta_step(opt_delta, rang_stock, rang_idle)

            sim.expiration_process.rate = round(opt_step[2] / K_exp, digit_round)
            algo.theta_step = opt_step
            ...
            if steps == tau_n:
                ...
                if sim_type == "plus_0":
                    ...
                    # Verify perturbations match between plus/minus pairs
                    perturb = np.array(data.get(f'perturbations_plus_0', perturbations.tolist()))

                    # Gradient via Algorithm
                    leader_algo.theta = np.array(data['opt'])
                    leader_algo.n = n
                    grad = leader_algo.compute_gradient(avg_costs_plus, avg_costs_minus, perturb)

                    # Optimizer update via Algorithm
                    new_opt = leader_algo.apply_optimizer(grad, optimization)

                    print(f"n = {n}, theta = {new_opt}, grad = {grad}, "
                          f"cost+ = {avg_costs_plus[0]:.4f}, cost- = {avg_costs_minus[0]:.4f}, "
                          f"tau_n = {tau_n}, delta_n = {delta_n:.6f}")

                    shared_data['opt'] = new_opt.tolist()
                    theta_rows.append({"opt": new_opt.tolist()})
                    theta_costs_rows.append({
                        "time": time.time(), "mode": sim_type,
                        "theta_stock": new_opt[0], "theta_idle": new_opt[1],
                        "theta_exp": new_opt[2], "n": n,
    ...
    print(f"Final theta: {shared_data['opt']}")
    ...
    np.savetxt(f"{log_dir}/all_states_{sim_type}.csv", np.array(sim.all_states) if sim.all_states else np.array([]), delimiter=',', fmt='%d')
...
# ======================================================================
# Config loading and experiment runner
# ======================================================================
...
def run_experiments_from_config(config_path):
    """Run parallel vectorial NSGD experiments from a JSON configuration file."""
    import json
    ...
    experiment_name = config.get('experiment_name', 'vectorial_par')
    ...
    for ti, theta_init in enumerate(theta_list):
        ...
        for seed in seeds:
            ...
            shared_data["opt"] = list(theta_init)
            shared_data["ready_count"] = 0
            for k in range(K):
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Parallel Vectorial NSGD Serverless Simulator')
