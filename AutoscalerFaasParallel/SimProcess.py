...
class ParetoSimProcess(SimProcess):
    """ParetoSimProcess extends the functionality of :class:`~simfaas.SimProcess.SimProcess` for
    Pareto distributed processes. This class also implements the `pdf` and `cdf` functions
    which can be used for visualization purposes.

    Parameters
    ----------
    shape : float
        The shape parameter (alpha) of the Pareto distribution
    scale : float, optional
        The scale parameter (x_min) of the Pareto distribution, default is 1.0
    """
    ...
    def generate_trace(self):
        ...
        # scipy.stats.pareto uses (x/scale)^(-shape-1) parameterization
