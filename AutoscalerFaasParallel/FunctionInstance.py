...
class FunctionInstance:
  id_counter = 0
  def __init__(self,t,cold_service_process, warm_service_process, expiration_process, cold_start_process, execution_time=0, ram=0,cpu=0,gpu=0,tpu=0,priority=1,deadline=None):
  ...
  def is_cold(self):
    return self.state.value == FunctionState.COLD.value
  def is_initializing(self):
    return self.state.value == FunctionState.INIT_FREE.value or self.state.value == FunctionState.INIT_RESERVED.value
  
  def is_busy(self):
    return self.state.value == FunctionState.BUSY.value
  
  def is_idle_on(self):
    return self.state.value == FunctionState.IDLE_ON.value
  
  def is_init_free(self):
    return self.state.value == FunctionState.INIT_FREE.value
  
  def is_init_reserved(self):
    return self.state.value == FunctionState.INIT_RESERVED.value
  
  def reserve(self):
    assert self.state.value == FunctionState.INIT_FREE.value, "Function must be in INIT_FREE state to be reserved."
    self.reserved = True
  ...
  def make_Init_Reserved(self):
    assert self.state.value == FunctionState.COLD.value, "Function must bein  COLD state to be init_reserved."
  ...
  def transition_state_to(self,state):
    assert state.value == FunctionState.INIT_FREE.value or state.value == FunctionState.INIT_RESERVED.value, "Cannot transition to INITIALIZING state."
    
    if state.value == FunctionState.IDLE_ON.value and self.is_initializing() and self.is_reserved():
        raise ValueError("A reserved instance cannot transition to IDLE_ON state.")
    self.state = state
      
  def generate_init_departure(self, t):
    #print("INIT",t, self.cold_start_process.generate_trace())
